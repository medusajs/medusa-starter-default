...
const SelectCustomerForm = ({ onSelect }: SelectCustomerFormProps) => {
  ...
  const handleRowClick = (event: React.MouseEvent, row: HttpTypes.AdminCustomer) => {
    ...
    // Navigate back to edit page with customer ID
    const returnTo = searchParams.get("return_to") || `/offers/${offerId}/edit`
  ...
  return (
    <RouteFocusModal>
      ...
      <span className={clx("sr-only")}>
        <RouteFocusModal.Title>Change Customer</RouteFocusModal.Title>
...
/**
 * Customer Selection Modal Page (Edit Flow)
 *
 * Modal route for changing the customer when editing an offer.
 * Follows MedusaJS pattern from customer-group-add-customers.
 */
const EditCustomerSelectionPage = () => {
  return (
    <RouteFocusModal>
      <SelectCustomerForm />
    </RouteFocusModal>
...
export default EditCustomerSelectionPage
