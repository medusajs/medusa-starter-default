...
let START_FROM_LINE = 71000; // Start from specified line
const BATCH_SIZE = 1000; // Smaller batches for memory management
const CHUNK_SIZE = 10000; // Process CSV in chunks
const MEMORY_LIMIT = 1500 * 1024 * 1024; // 1.5GB memory limit
...
// Read CSV header from first line
...
function processRecord(record: any, partsMap: Map<string, LightweightPart>, logger: any): void {
  ...
  if (price === null || !createdAt) {
    if (!REDUCE_LOGGING) {
      logger.warn(`Skipping record with invalid price or date: part_number=${partNumber}, price=${record.price_value}, date=${record.price_created_at}`);
    }
    return;
  }
    ...
    try {
      ...
      const productData = {
        ...
        metadata: {
          ...
          part_color_id: part.part_color_id,
          total_price_changes: part.price_count,
          import_source: 'parts-csv-import-optimized-fixed',
...
export default async function loadPartsWithPricesOptimizedFixed({ container }: ExecArgs) {
  const logger = container.resolve(ContainerRegistrationKeys.LOGGER);
  const salesChannelModuleService = container.resolve(Modules.SALES_CHANNEL);
  const productModuleService = container.resolve(Modules.PRODUCT);

  logger.info("Starting optimized parts import process for 1.5M records (FIXED VERSION)...");

  try {
    // Use START_FROM_LINE as the primary configuration
    let startLine = START_FROM_LINE;
    let createdCount = 0;
    let skippedCount = 0;

    // Only load progress if START_FROM_LINE is 1 (starting from beginning)
    if (START_FROM_LINE === 1) {
      const savedProgress = loadProgress();
      if (savedProgress) {
        logger.info(`Resuming from saved progress at line ${savedProgress.lastProcessedLine}`);
        startLine = savedProgress.lastProcessedLine;
        createdCount = savedProgress.createdCount;
        skippedCount = savedProgress.skippedCount;
      }
    } else {
      logger.info(`Starting from specified line ${START_FROM_LINE} (ignoring any saved progress)`);
    }
    ...
    // CRITICAL FIX: Always read header from line 1, regardless of start line
    logger.info("Reading CSV header from line 1...");
    ...
    // Stream processing for data lines
    const fileStream = createReadStream(csvPath);
    const rl = createInterface({
      input: fileStream,
      crlfDelay: Infinity
    ...
    for await (const line of rl) {
      ...
      // Skip header line and lines before start line
      if (lineNumber === 1 || lineNumber < startLine) continue;
      
      // Process data lines
      const values = parseCSVLine(line);
      if (values.length !== header.length) {
        if (!REDUCE_LOGGING) {
          logger.warn(`Skipping line ${lineNumber} with incorrect number of columns: expected ${header.length}, got ${values.length}`);
        }
        continue;
      }
      ...
      processRecord(record, partsMap, logger);
      ...
      // Save progress periodically (only if starting from beginning)
      if (START_FROM_LINE === 1 && lineNumber % PROGRESS_INTERVAL === 0) {
    ...
    // Final progress save (only if starting from beginning)
    if (START_FROM_LINE === 1) {
